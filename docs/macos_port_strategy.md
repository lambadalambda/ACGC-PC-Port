...
- the current jaudio DSP/channel pointer-packing sites in `src/static/jaudio_NES/internal/oneshot.c`, `src/static/jaudio_NES/internal/driverinterface.c`, `src/static/jaudio_NES/internal/ipldec.c`, `src/static/jaudio_NES/internal/dspbuf.c`, and `src/static/jaudio_NES/internal/dspinterface.c` now use the same checked helper too, with `pc/tests/check_audio_dsp_runtime_ptr_contract.sh` locking the current source contract
- jaudio bank/wave/system pointer packing in `src/static/jaudio_NES/internal/system.c` and `src/static/jaudio_NES/internal/memory.c` now uses checked narrowing or `uintptr_t` arithmetic for the current bringup paths, with `pc/tests/check_audio_system_runtime_ptr_contract.sh` and `pc/tests/check_audio_simple_runtime_ptr_contract.sh` locking the current source contract
- the active jaudio driver-side sample and table pointer packing in `src/static/jaudio_NES/internal/driver.c` now uses the same checked narrowing or `uintptr_t` arithmetic, with `pc/tests/check_audio_driver_runtime_ptr_contract.sh` locking the current source contract
- jaudio callback payload reconstruction now makes the 32-bit audio-port contract explicit in `src/static/jaudio_NES/internal/sub_sys.c`, and the odd callback-tag handling in `src/static/jaudio_NES/internal/driver.c` now masks through `uintptr_t` instead of truncating through `u32`; `pc/tests/check_audio_driver_runtime_ptr_contract.sh` locks that contract too
- jaudio bank overwrite commands now reconstruct 32-bit pointer payloads explicitly in `src/static/jaudio_NES/internal/channel.c`, with `pc/tests/check_audio_bank_runtime_ptr_contract.sh` locking the current source contract
- jaudio bank/external-pointer command payload APIs now use explicit `u32` parameters in `src/static/jaudio_NES/internal/channel.c` and `src/static/jaudio_NES/internal/system.c`, with `src/static/jaudio_NES/internal/sub_sys.c` passing `port->param.asU32`; `pc/tests/check_audio_command_payload_contract.sh` locks that contract too
- guarded LP64 bank relocation in `src/static/jaudio_NES/internal/system.c` now deserializes 32-bit bank entries into host-side shadow `voiceinfo` storage instead of overlaying pointer-bearing native structs on serialized bank blobs, and `pc/tests/check_audio_bank_runtime_ptr_contract.sh` now locks that LP64 shadow-storage contract too
- `src/static/JSystem/JKernel/JKRExpHeap.cpp` now keeps tail-allocation pointer arithmetic in `uintptr_t` until the final reconstruction step, with `pc/tests/check_jkr_exp_heap_runtime_ptr_contract.sh` locking that contract
