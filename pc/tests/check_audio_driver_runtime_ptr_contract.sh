...
check_contains "src/static/jaudio_NES/internal/driver.c" 'u32 sampleDmaSrc = PC_RUNTIME_U32_PTR\(tmpSamplesToLoadAddr\);' 'driver DMA callback source narrowing'
check_contains "src/static/jaudio_NES/internal/driver.c" '\(u8\*\)Nas_WaveDmaCallBack\(sampleDmaSrc, sampleDataChunkSize, flags, &driver->sample_dma_idx,' 'driver DMA callback source address'
...
check_contains "src/static/jaudio_NES/internal/driver.c" 'aLoadBuffer2\(cmd\+\+, VELOCONV_TABLE\[vel_conv_idx\], 0x800,' 'driver velocity table load'
...
check_contains "src/static/jaudio_NES/internal/driver.c" 'Nas_LoadBuffer2\(cmd\+\+, DMEM_COMB_TEMP - combFilterSize, combFilterSize, combFilterState\);' 'driver comb filter state load'
check_contains "src/static/jaudio_NES/internal/driver.c" 'Nas_SaveBuffer2\(cmd\+\+, DMEM_TEMP - combFilterSize \+ size, combFilterSize, combFilterState\);' 'driver comb filter state save'
check_contains "src/static/jaudio_NES/internal/driver.c" 'Nas_PCM8dec\(cmd\+\+, flags, driver->synth_params->adpcm_state\);' 'driver adpcm state forwarding'
check_contains "src/static/jaudio_NES/internal/driver.c" '#define JAUDIO_CALLBACK_TAG\(value\) \(\(u32\)\(\(uintptr_t\)\(value\) & 0xFFu\)\)' 'driver callback tag helper'
...
check_contains "src/static/jaudio_NES/internal/driver.c" 'u32 reverbAddrTag;' 'driver reverb callback tag storage'
...
check_contains "src/static/jaudio_NES/internal/driver.c" '\(const void\*\)\(uintptr_t\)reverbAddrTag' 'driver reverb load tag cast'
check_contains "src/static/jaudio_NES/internal/sub_sys.c" '#define JAUDIO_U32_CALLBACK\(type, value\) \(\(type\)\(uintptr_t\)\(value\)\)' 'subsys callback reconstruction helper'
check_contains "src/static/jaudio_NES/internal/sub_sys.c" 'NA_VFRAME_CALLBACK = JAUDIO_U32_CALLBACK\(VFRAME_CALLBACK, Nap_PortGetPtr\(port\)\);' 'subsys vframe callback restore'
check_contains "src/static/jaudio_NES/internal/sub_sys.c" 'NA_SOUND_CALLBACK = JAUDIO_U32_CALLBACK\(SOUND_CALLBACK, Nap_PortGetPtr\(port\)\);' 'subsys sound callback restore'
check_contains "src/static/jaudio_NES/internal/sub_sys.c" 'NA_DACOUT_CALLBACK = JAUDIO_U32_CALLBACK\(DACOUT_CALLBACK, Nap_PortGetPtr\(port\)\);' 'subsys dac callback restore'
check_contains "src/static/jaudio_NES/internal/sub_sys.c" 'AG.seq_callbacks\[port->command.arg2\] = JAUDIO_U32_CALLBACK\(SequenceCallback, Nap_PortGetPtr\(port\)\);' 'subsys sequence callback restore'
