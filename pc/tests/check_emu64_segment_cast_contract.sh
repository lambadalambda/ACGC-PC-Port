...
if ! rg -q 'now_setimg\.setimg2\.imgaddr = this->seg2k0\(setimg2->imgaddr\);' "$EMU64_FILE"; then
...
if ! rg -q 'u32 resolved = this->segments\[seg\] \+ offset;' "$UTILITY_FILE"; then
...
if ! rg -q 'k0 = this->segments\[\(segadr >> 24\) & 0xF\] \+ \(segadr & 0xFFFFFF\);' "$UTILITY_FILE"; then
