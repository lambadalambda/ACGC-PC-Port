...
"""Run Animal Crossing with automatic soft-hang detection for repro capture.

This watchdog intentionally uses multiple soft-hang signals because the common
startup/progression stalls do not present identically on every run/build:
...
The goal is to stop reproducible hangs without requiring manual force-quit.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Animal Crossing and auto-terminate on reproducible soft-hang signatures."
    )
...
def ingest_line(state: WatchdogState, args: argparse.Namespace, line: str, now: float) -> None:
    ...
    if SCENE_GATE_TOKEN in line:
        ...
        # Prefer arming from the explicit scene gate when available.
...
def main() -> int:
    ...
    if stop_kind == "hang":
        print(f"[WATCHDOG] Detected soft-hang signature: {stop_reason}; process terminated.")
